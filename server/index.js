  ...
  r.on("message", function (n) {
    ...
    initialized ||
      (new rxjs_1.Observable(function (n) {
        // Every 100ms new data is generated calling generateSensor
        ...
        .pipe( //to chain the operators 
          operators_1.concatMap(function (e) { 
            // to map each emitted item from the source observable to a new observable
            // the following delays randomly the emission of items from the source observable
            return rxjs_1.of(e).pipe(operators_1.delay(5 + 5 * Math.random()));
          })
        )
        .subscribe(function (n) {
          // retrieves the data by subscribing to the observable and send it with ws
          return (
